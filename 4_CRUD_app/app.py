...
@app.route('/todos/<todo_id>/set-completedd', methods=['POST'])
def set_completedd_todo(todo_id):
    error = False

    try:
        completedd = request.get_json()['completedd']

        todo = Todo.query.get(todo_id)
        todo.completedd = completedd
...
@app.route('/lists/<list_id>/set-completedd', methods=['POST'])
def set_completedd_list(list_id):
    error = False
